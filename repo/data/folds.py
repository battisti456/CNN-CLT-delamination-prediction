...
class D2(torch.utils.data.Dataset):
    def __init__(
            self,
            hp:Hyperparameters,
        ...
        self.signal = get_boosted_in_data(hp,base_indices,extend_by)
        ...
        if append_poster:
            signal = np.reshape(_data['ultrasound_poster'],(5*52,SIGNAL_LENGTH))
            ...
            signal = _preprocess_signals(hp,signal)
            num_layers = np.empty_like(labels)
            num_layers[:] = 2
            self.signal = np.concat((self.signal,signal), axis = 0)
            self.labels = np.concat((self.labels,labels), axis = 0)
            self.num_layers = np.concat((self.num_layers,num_layers), axis = 0)
        fft =  np.fft.rfft(self.signal,axis = -1)
        self.fft = np.moveaxis(np.array(
            [np.real(fft),np.imag(fft)]
        ),(0,1,2),(1,0,2))
    def __getitem__(self,idx):
        to_return = {}
        if 'signal' in self.hp.use_parameters:
            to_return['signal'] = torch.tensor(self.signal[idx,None,:],dtype = torch.float32)
        if 'fft' in self.hp.use_parameters:
            to_return['fft'] = torch.tensor(self.fft[idx,...],dtype = torch.float32)
        if 'num_layers' in self.hp.use_parameters:
            to_return['num_layers'] = torch.tensor(self.num_layers[idx],dtype = torch.int8)
        if 'spectrogram' in self.hp.use_parameters:
            to_return['spectrogram'] = torch.tensor(
                spectrogram(self.hp,self.signal[idx,None,...]),
                dtype=torch.float32
            )
        return to_return,self.labels[idx]
...
def get_folds(hp:Hyperparameters) -> Generator[tuple[
    ...
    for fold in range(hp.num_folds):
        ...
        if hp.data_balance_mode == 'balance':
            ...
            raise NotImplementedError()
        yield (
            D2(hp,train_indices,extend_by, append_poster=hp.append_poster_to_train and hp.out_type == 'edge' and hp.out_shape == 'block'),
            D2(hp,test_indices,NUM_REPLICATES)
        )